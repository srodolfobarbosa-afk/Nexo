...
def propose_and_submit(repo: str, file_path: str, intent_description: str, auto_apply: bool = False):
    ...
    if auto_apply:
        req = AgentRequest(repo=repo, branch_prefix='autogen', file_path=file_path, file_content=proposal['proposed_content'], pr_title=f"Auto change: {file_path}", pr_body=intent_description, auto_apply=True)
