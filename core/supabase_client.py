...
def save_task(table_name: str, data: dict):
    ...
    if not supabase:
        ...
        try:
            sqlite_client.save_task_local(data.get('name', 'task'), status=data.get('status','pending'), result=str(data.get('result', '')), reward=float(data.get('reward',0.0)))
        except Exception as e:
...
def save_log(level: str, message: str, details: dict = None):
    ...
    if not supabase:
        ...
        try:
            sqlite_client.save_log_local(level, message, str(details) if details else None)
        except Exception as e:
