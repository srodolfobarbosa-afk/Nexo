...
class SelfCorrectionModule:
    ...
    def initialize_supabase(self):
        if not self.supabase_url or not self.supabase_key:
            logger.error(
                "SUPABASE_URL ou SUPABASE_KEY não configurados para SelfCorrectionModule."
            )
            return
        try:
            ...
            logger.error(
                f"Erro ao conectar com Supabase para SelfCorrectionModule: {e}"
            )
    ...
    def log_error(self, error_type: str, description: str, context: dict = None):
        ...
        try:
            data, count = (
                self.supabase.table(self.error_log_table)
                .insert(
                    {
            ...
            logger.error(
                f"Erro logado para {self.agent_name}: {error_type} - {description}"
            )
        except Exception as e:
            logger.error(f"Erro ao logar erro no Supabase: {e}")

    def add_to_learning_memory(
        self, lesson: str, action_taken: str, effectiveness: str, context: dict = None
    ):
        if not self.supabase:
            logger.warning(
                "Supabase não inicializado. Não foi possível adicionar à memória de aprendizado."
            )
            return
        try:
            data, count = (
            ...
            logger.info(
                f"Lição aprendida adicionada à memória para {self.agent_name}: {lesson}"
            )
        except Exception as e:
    ...
    def retrieve_learning_lessons(self, query: str = None, limit: int = 5) -> list:
        if not self.supabase:
            logger.warning(
                "Supabase não inicializado. Não foi possível recuperar lições."
            )
            return []
        try:
    ...
    def reflect_on_performance(self, user_feedback: str, current_context: dict = None):
        ...
        logger.info(
            f"Iniciando auto-reflexão com base no feedback do usuário: {user_feedback}"
        )

        # Exemplo de lição aprendida com base no feedback do usuário
        if (
            "esquecer" in user_feedback.lower()
            or "memória" in user_feedback.lower()
            or "contexto" in user_feedback.lower()
        ):
        ...
        if (
            "discurso" in user_feedback.lower()
            or "ação real" in user_feedback.lower()
            or "prático" in user_feedback.lower()
        ):
        ...
        logger.info(
            "Auto-reflexão concluída. Lições adicionadas à memória de aprendizado."
        )
...
if __name__ == "__main__":
    ...
    # Simular um erro
    correction_module.log_error(
        "ContextLoss",
        "Perda de contexto em interação crítica",
        {"user_message": "...", "agent_response": "..."},
    )

    # Simular feedback do usuário e reflexão
    feedback = "Você esqueceu o que conversamos! Precisa de mais ação real."
    correction_module.reflect_on_performance(
        feedback, {"current_task": "Implementar comunicação integrada"}
    )
    ...
    for lesson in lessons:
        logging.info(
            f"Lição: {lesson['lesson']}\nAção: {lesson['action_taken']}\nEficácia: {lesson['effectiveness']}\n"
        )

    lessons_about_context = correction_module.retrieve_learning_lessons(
        query="contexto"
    )
