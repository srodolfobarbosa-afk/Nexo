...
class InternetSearchModule:
    ...
    def search_code_examples(self, technology, problem):
        ...
        for result in results:
            if any(
                site in result["link"]
                for site in ["github.com", "stackoverflow.com", "docs."]
            ):
                content = self.scrape_content(result["link"])
                if content:
                    code_results.append({**result, "content": content["content"]})
    ...
    def search_documentation(self, library_name):
        ...
        for result in results:
            if any(
                keyword in result["link"]
                for keyword in ["docs.", "documentation", "readthedocs"]
            ):
                content = self.scrape_content(result["link"])
                if content:
                    doc_results.append({**result, "content": content["content"]})
