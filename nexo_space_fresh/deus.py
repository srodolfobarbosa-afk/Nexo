...
class HuggingFaceManager:
    def __init__(self):
        ...
        self.hf_api = HfApi() if self.hf_token else None
        self.user_info = None
        if self.hf_token:
            try:
                login(token=self.hf_token)
                self.user_info = self.hf_api.whoami()
    ...
    def download_model(self, model_id: str) -> Optional[str]:
        try:
            ...
            path = hf_hub_download(repo_id=model_id)
...
async def startup_event():
    ...
    logger.info(f"✅ HF: {'Autenticado' if hf_manager.hf_token else '❌ Faltando'}")
