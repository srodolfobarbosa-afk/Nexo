...
class IntegratedCommunicationSystem:
    ...
    def process_integrated_message(self, message: str) -> List[str]:
        ...
        for response in responses:
            self.conversation_history.append(
                {"agent_response": response, "timestamp": datetime.now().isoformat()}
            )
...
def handle_mode_change(call):
    ...
    if comm_system.set_conversation_mode(mode):
        ...
        bot.edit_message_text(
            "❌ Modo inválido", call.message.chat.id, call.message.message_id
        )
...
def manage_agents(message):
    ...
    markup.row(
        ...
        types.InlineKeyboardButton(
            f"Manus {manus_status}", callback_data="toggle_manus"
        ),
    )
...
def handle_integrated_message(message):
    ...
    try:
        ...
        if not responses:
            bot.send_message(
                message.chat.id, "⚠️ Nenhum agente ativo para processar a mensagem"
            )
            return
        ...
        bot.send_message(
            message.chat.id, "❌ Erro ao processar mensagem. Tente novamente."
        )
