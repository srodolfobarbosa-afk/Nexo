...
def test_supabase_integration():
    ...
    try:
        data = supabase.table('nexo_log').insert({'mensagem': 'Teste de integracao Nexo', 'tipo': 'teste', 'resultado': 'ok'}).execute()
        ...
        try:
            ...
            if isinstance(e, _API) or (hasattr(e, 'args') and e.args and isinstance(e.args[0], dict) and e.args[0].get('code') in ('PGRST205', 'PGRST204')):
                pytest.skip(f"Supabase schema not ready or table missing: {getattr(e, 'args', e)}")
        except Exception:
            pass
