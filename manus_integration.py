...
class ManusIntegration:
    def __init__(self):
        ...
        if not self.api_key:
            logger.info(
                "API do Manus ainda não está publicamente disponível. Aguardando lançamento oficial."
            )
        else:
...
if __name__ == "__main__":
    ...
    if manus_integration is None:
        print(
            "Teste não pode ser executado. Por favor, configure a variável de ambiente MANUS_API_KEY."
        )
    else:
        # Exemplo 1: Criar uma tarefa real de análise
        print("\nChamando a API para uma tarefa de 'análise'...")
        manus_response = manus_integration.create_task(
            "Por favor, analise os dados de vendas do último trimestre."
        )
        ...
        automacao_response = manus_integration.create_task(
            "Automatizar o processo de envio de relatórios semanais."
        )
        print("Resposta da API Manus:")
